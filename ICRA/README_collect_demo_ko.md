...
| 수집 루프 | 성공 1개만 저장 후 종료 | robosuite 기본 방식: 성공 에피소드를 계속 누적 저장 |
...
| `--directory` | `./demos` | 저장 루트. 실제 경로는 `./demos/<Env>_<Robot>/demo.hdf5` |

## 저장 형식

`gather_demonstrations_as_hdf5` 를 그대로 재사용하므로 기존과 동일한 HDF5
구조(`data/demo_i/{states, actions}`, `data.attrs.env_info`)로 저장됩니다.
`env_info` JSON 에 `action_dim`, `arm_controller` 가 기록되어 다운스트림에서
액션 차원을 검증할 수 있습니다.
